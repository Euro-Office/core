...
namespace agg
{
	...
	private:
		...
		{
			int x_index = (int)(RES - 1) * (x - xmin) / (xmax - xmin);
			int y_index = (int)(RES - 1) * (y - ymin) / (ymax - ymin);
	...
	protected:
			...
			for (int i = 0; i < RES; ++i)
			{
				...
				for (int j = 0; j < RES; ++j)
				{
					...
					if (i == 0 || j == 0)
					{
						...
						index1 = get_index_curve<int>(p.x, p.y);
					}
					else
					{
						index1 = next_indexes[j];
					...
					std::pair<int, int> index2 = get_index_curve<int>(p.x, p.y);
					cur_next_indexes[j + 1] = index2;
		...
		{
			auto i = get_index_curve<size_t>(x, y);
			if (i.first > RES - 1 || i.second > RES - 1 || i.first < 0 || i.second < 0)
				return {0, 0, 0, 0};
		...
		template<typename T>
		std::pair<T, T> get_index_curve(float x, float y)
		{
			T x_index = (T)(RES - 1) * (x - xmin_curve) / (xmax_curve - xmin_curve);
			T y_index = (T)(RES - 1) * (y - ymin_curve) / (ymax_curve - ymin_curve);
