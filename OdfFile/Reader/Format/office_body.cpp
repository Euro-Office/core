		...
		bool bFirstPageOnly = false;
		{
			...
			for (size_t i = 0; i < masterPagesAll.size(); ++i)
			{
				...
				if (!lpName.empty())
				{
					...
					if (pl && pl->properties() &&
						(pl->properties()->attlist_.common_background_color_attlist_.fo_background_color_ ||
						 pl->properties()->style_background_image_))
					{
						backgroundLayout = pl;
						break;
					}
			...
			if (!backgroundLayout)
			{
				// Find the first non-transitional master to use as the "default" target.
				// ODF has no formal default; the cover master may legally appear at index 0.
				std::wstring defaultMasterName;
				for (size_t i = 0; i < masterPagesAll.size(); ++i)
				{
					if (!masterPagesAll[i]) continue;
					std::wstring nm = masterPagesAll[i]->attlist_.style_name_.get_value_or(L"");
					bool isTransitional = false;
					if (masterPagesAll[i]->attlist_.style_next_style_name_)
					{
						const std::wstring & ns = *masterPagesAll[i]->attlist_.style_next_style_name_;
						isTransitional = !ns.empty() && ns != nm;
					}
					if (!isTransitional) { defaultMasterName = nm; break; }
				}
				// Fallback: if all masters are transitional, use first entry.
				if (defaultMasterName.empty() && !masterPagesAll.empty() && masterPagesAll[0])
					defaultMasterName = masterPagesAll[0]->attlist_.style_name_.get_value_or(L"");

				for (size_t i = 0; i < masterPagesAll.size(); ++i)
				{
					if (!masterPagesAll[i]) continue;
					std::wstring mpName = masterPagesAll[i]->attlist_.style_name_.get_value_or(L"");
					if (mpName == defaultMasterName)
						continue; // this IS the default master, not a cover
					if (!masterPagesAll[i]->attlist_.style_next_style_name_)
